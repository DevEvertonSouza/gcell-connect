...
type Brand = { id: string; name: string; logo_url?: string };
...
const StoreBrands = () => {
  ...
  const add = async () => {
    ...
    if (error) {
      toast.error(error.code === "23505" ? "Marca já existe." : "Erro ao criar marca.");
    } else {
              ...
              {editId === b.id ? (
                <div className="flex items-center gap-2 flex-1">
                  <Input
                    ...
                    className="bg-background border-border"
                    onKeyDown={(e) => e.key === "Enter" && update(b.id)}
                  />
                  <Button size="icon" variant="ghost" onClick={() => update(b.id)}>
